...
public class LotteryController : BaseController
{
    ...
    [Route("check-multi")]
    [ValidateAntiForgeryToken]
    public async Task<IActionResult> CheckMultipleTickets(string LotteryNumbers, DateTime DrawDate)
    {
        if ((LotteryNumbers == null && DrawDate == null) || string.IsNullOrWhiteSpace(LotteryNumbers))
        {
        ...
        {
            var lotteryResponse = await HttpUtils<LotteryVm>.SendRequest(HttpMethod.Get,
                $"{Constants.API_LOTTERY}/get-lottery-result?year={DrawDate.Year}&month={DrawDate.Month}&day={DrawDate.Day}");
            ViewData["LotteryResult"] = lotteryResponse.Data?.Result?.GroupBy(l => l.RewardId).OrderByDescending(g => g.Key);

            var lottery = LotteryNumbers.Split(',');

            var results = new Dictionary<string, RewardVm?>();

            var userData = TempData["User"].ToString();
            var user = JsonConvert.DeserializeObject<UserVm>(userData);
            var multiple = new MultipleLotteryNumbersVm
            {
            ...
            var searchResponse = await HttpUtils<RewardVm>.SendRequest(HttpMethod.Post,
                $"{Constants.API_LOTTERY}/get-multiple-ticket-results", multiple);
            foreach (var result in searchResponse.Data?.Result)
            {
            ...
            if (user != null)
            {
                foreach (var rst in results)
                {
                    var addSearchHistoryResponse = await HttpUtils<SearchHistoryVm>.SendRequest(HttpMethod.Post,
                        $"{Constants.API_SEARCH_HISTORY}/create-search-history", new SearchHistoryVm()
                        {
                            LotteryNumber = rst.Key,
                            SearchDate = DateTime.Now,
                            UserId = user.Id,
                            DrawDate = DrawDate,
                            Email = user.Email,
                            Prize = rst.Value?.RewardValue ?? 0
                        }, accessToken: Request.Cookies["AccessToken"]);
                }
            }
            return RedirectToAction("CheckTicket", new
